...
const OPERATIONS = [
  { label: 'HTML Conversion', value: 'html', icon: 'i-lucide-code', description: 'Convert PDF to a full HTML string' },
  { label: 'Text Extraction', value: 'text', icon: 'i-lucide-file-text', description: 'Extract plain text content' },
  { label: 'Pages (per-page)', value: 'pages', icon: 'i-lucide-layout', description: 'Get HTML or text per page' },
  { label: 'Metadata', value: 'meta', icon: 'i-lucide-info', description: 'Extract title, author, dates, etc.' },
  { label: 'Thumbnail', value: 'thumbnail', icon: 'i-lucide-image', description: 'Generate a page preview image' },
  { label: 'Extract Images', value: 'images', icon: 'i-lucide-images', description: 'Extract all embedded images' }
] as const
...
async function submit() {
  if (!selectedFile.value) {
    toast.add({ title: 'No file selected', description: 'Please upload a PDF file first.', color: 'error', icon: 'i-lucide-alert-circle' })
    return
  }
  if (!apiKey.value.trim()) {
    toast.add({ title: 'No API key', description: 'Enter an API key to authenticate.', color: 'error', icon: 'i-lucide-key' })
    return
  }
  ...
  try {
    ...
    if (res.success) {
      ...
      toast.add({ title: 'Success', description: `${selectedOp.value.label} completed.`, color: 'success', icon: 'i-lucide-check-circle' })
    }
    else {
...
<template>
  <UContainer class="py-10 max-w-4xl">
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-highlighted mb-1">
        p2h-service
      </h1>
            ...
            <template #default="{ open }">
              ...
              >
                <template v-if="selectedFile">
                  <UIcon name="i-lucide-file-check" class="size-8 text-success mx-auto mb-2" />
                  <p class="text-sm font-medium text-highlighted truncate max-w-full">
                    {{ selectedFile.name }}
                  </p>
                ...
                <template v-else>
                  <UIcon name="i-lucide-file-up" class="size-8 text-muted mx-auto mb-2" />
        ...
        <UCard>
          ...
          <div class="flex flex-col gap-2">
            <div
              ...
              :class="operation === op.value ? 'bg-primary/10 text-primary' : 'hover:bg-elevated'"
          ...
          <div v-if="operation === 'thumbnail'" class="flex flex-col gap-3">
            ...
            <div class="flex items-center gap-3">
              ...
              <USelect
                ...
                :items="[{ label: 'PNG', value: 'png' }, { label: 'JPG', value: 'jpg' }]"
                value-key="value"
                class="flex-1"
              />
            </div>
            <div class="flex items-center gap-3">
              ...
              <UInputNumber v-model="thumbnailWidth" :min="1" placeholder="Default" class="flex-1" />
            </div>
            <div class="flex items-center gap-3">
              <span class="text-sm w-24 shrink-0">Height (px)</span>
              <UInputNumber v-model="thumbnailHeight" :min="1" placeholder="Default" class="flex-1" />
            </div>
          </div>
        </UCard>
        ...
        <UCard class="h-full min-h-96">
          ...
          <div v-else class="space-y-4">
            ...
            <template v-else-if="operation === 'text' && result.text">
              ...
              <pre class="p-4 bg-elevated rounded-md text-sm max-h-[600px] overflow-auto whitespace-pre-wrap border border-default">{{ result.text }}</pre>
            </template>

            <!-- Pages result -->
            <template v-else-if="operation === 'pages' && Array.isArray(result.pages)">
              <div class="text-xs text-muted mb-2">
                {{ (result.pages as string[]).length }} page(s) — mode: {{ result.mode }}
              </div>
              <UAccordion
                :items="(result.pages as string[]).map((page, i) => ({
                  label: `Page ${i + 1}`,
                  content: page
                }))"
                type="multiple"
              >
                <template #content="{ item }">
                  ...
                  <pre
                    v-else
                    class="p-3 text-sm whitespace-pre-wrap"
                  >{{ item.content }}</pre>
                </template>
              </UAccordion>
            </template>
            ...
            <template v-else-if="operation === 'meta' && result.meta">
              ...
              <div class="divide-y divide-default rounded-md border border-default overflow-hidden">
                <div
                  v-for="[k, v] in Object.entries(result.meta as Record<string, unknown>)"
                  :key="k"
                  class="flex gap-3 px-3 py-2 text-sm hover:bg-elevated"
                >
                  <span class="font-mono text-xs text-muted w-48 shrink-0 truncate pt-0.5">{{ k }}</span>
            ...
            <!-- Images result -->
            <template v-else-if="operation === 'images' && Array.isArray(result.images)">
              <div class="text-xs text-muted mb-2">
              ...
              <div v-else class="grid grid-cols-2 gap-3">
                <div
                  v-for="(img, i) in (result.images as { data: string, mimeType: string }[])"
                  :key="i"
